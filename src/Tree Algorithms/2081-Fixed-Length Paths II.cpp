...
struct centroid_dec
{
	...
	centroid_dec(vector < vector < int > > &gi, int K1i, int Ki) : g(gi) , N(gi.size()) , K1(K1i) , K(Ki) , Di(Ki+1)
	{
		dad.resize(N);
		siz.resize(N);
		isCe.resize(N);
